...
class NeuralConquestController {
  ...
  static async generateNeuralConquestTopics(req, res) {
    try {
      ...
      const optimalCount = NeuralConquestController.determineOptimalTopicCount(topicDescription, 7);
...
🎯 REMEMBER: The Description field is the most critical part - it becomes the Shap-E prompt that generates the 3D model. Make each one detailed, specific, and visually descriptive for optimal 3D generation results.`;
      ...
      const aiResponse = await geminiService.generateContent(prompt, 5);
      ...
      const parsedData = NeuralConquestController.parseNeuralConquestResponse(aiResponse);

      if (!parsedData.topics || parsedData.topics.length === 0) {
        console.warn('⚠️ AI parsing failed, creating enhanced fallback topics');
        const fallbackData = NeuralConquestController.createEnhancedFallbackTopics(topicDescription, optimalCount);
        ...
        return res.json({
          ...
          data: {
            ...
            subjectArea: subjectArea || NeuralConquestController.inferMainSubject(topicDescription),
      ...
      const fallbackCount = NeuralConquestController.determineOptimalTopicCount(req.body.topicDescription, 7);
      const fallbackData = NeuralConquestController.createEnhancedFallbackTopics(req.body.topicDescription, fallbackCount);
      return res.json({
        success: true,
        data: {
          topics: fallbackData.topics,
          ...
          subjectArea: NeuralConquestController.inferMainSubject(req.body.topicDescription),
  ...
  // Generate 3D models for selected topics (called after user selection)
  static async generate3DModelsForSelectedTopics(req, res) {
    try {
      ...
      const objectsData = selectedTopics.map((topic, i) => ({
        ...
        visualStyle: NeuralConquestController.determineVisualStyle(topic.concept, topic.difficulty),
      ...
      for (let i = 0; i < selectedTopics.length; i++) {
        ...
        if (modelResult.success) {
          const enhancedTopic = {
            ...
            model: {
              position: NeuralConquestController.calculateSpherePosition(i, selectedTopics.length),
            ...
            color: NeuralConquestController.getConceptColor(topic.concept)
          };
          ...
          generationErrors.push({ topic: topic.name, error: modelResult.error, index: i });
  ...
  // Helpers specific to Neural Conquest
  static determineOptimalTopicCount(description, targetCount) {
    const lowerDesc = description.toLowerCase();
    if (lowerDesc.includes('continents')) return 7;
    if (lowerDesc.includes('oceans')) return 5;
    if (lowerDesc.includes('solar system') || lowerDesc.includes('planets')) return 8;
    if (lowerDesc.includes('world war')) return 6;
    if (lowerDesc.includes('calc') || lowerDesc.includes('calculus')) return 12;
    if (lowerDesc.includes('algebra')) return 10;
    if (lowerDesc.includes('geometry')) return 8;
    if (lowerDesc.includes('chemistry') && lowerDesc.includes('periodic')) return 18;
    if (lowerDesc.includes('biology') && lowerDesc.includes('systems')) return 11;
    if (lowerDesc.includes('math') || lowerDesc.includes('calculus') || lowerDesc.includes('algebra')) {
      return Math.min(15, Math.max(8, targetCount));
    }
    if (lowerDesc.includes('geography') || lowerDesc.includes('countries')) {
      return Math.min(10, Math.max(5, targetCount));
    }
    if (lowerDesc.includes('history')) {
      return Math.min(8, Math.max(5, targetCount));
    }
    if (lowerDesc.includes('science')) {
      return Math.min(12, Math.max(6, targetCount));
    }
    return Math.min(12, Math.max(5, targetCount));
  }

  static createEnhancedFallbackTopics(description, count) {
    const mainSubject = NeuralConquestController.inferMainSubject(description);
    const topics = [];
    const questions = [];

    const topicTemplates = NeuralConquestController.getTopicTemplates(mainSubject, description);
  ...
  static parseNeuralConquestResponse(aiResponse) {
    ...
    try {
      ...
      if (sections.length >= 2) {
        ...
        for (const block of topicBlocks) {
          ...
          for (const line of lines) {
            if (line.startsWith('Name:')) topic.name = line.replace('Name:', '').trim();
            else if (line.startsWith('Description:')) topic.description = line.replace('Description:', '').trim();
            else if (line.startsWith('Concept:')) topic.concept = line.replace('Concept:', '').trim();
            else if (line.startsWith('Difficulty:')) topic.difficulty = parseInt(line.replace('Difficulty:', '').trim()) || 1;
            else if (line.startsWith('Cost:')) topic.cost = parseInt(line.replace('Cost:', '').trim()) || 500;
            else if (line.startsWith('Educational Value:')) topic.educationalValue = line.replace('Educational Value:', '').trim();
          }
          if (topic.name && topic.description) {
            topic.icon = NeuralConquestController.generateTopicIcon(topic.name, topic.description, topic.concept);
            topics.push(topic);
          }
        }
        ...
        for (const block of questionBlocks) {
          ...
          for (const line of lines) {
            if (line.startsWith('Q:')) question.question = line.replace('Q:', '').trim();
            else if (line.match(/^[A-D]\)/)) {
              const option = line.substring(2).trim();
              const letter = line.charAt(0);
              question.options.push({ letter, text: option });
            } else if (line.startsWith('Correct:')) question.correct = line.replace('Correct:', '').trim();
            else if (line.startsWith('Difficulty:')) question.difficulty = parseInt(line.replace('Difficulty:', '').trim()) || 1;
            else if (line.startsWith('Topic:')) question.topic = line.replace('Topic:', '').trim();
          }
      ...
      console.warn('⚠️ Error parsing AI response, using fallback parsing', e);
      const fallbackTopics = NeuralConquestController.extractFallbackTopics(aiResponse);
  ...
  static generateTopicIcon(name, description, concept) {
    ...
    const conceptIcons = { geography: '🌍', history: '🏛️', science: '🔬', technology: '💻', mathematics: '📐', art: '🎨', biology: '🧬', general: '🏆' };
  ...
  static inferMainSubject(description) {
    const lowerDesc = description.toLowerCase();
    if (lowerDesc.includes('geography') || lowerDesc.includes('continents') || lowerDesc.includes('countries') || lowerDesc.includes('mountains') || lowerDesc.includes('rivers') || lowerDesc.includes('oceans')) return 'geography';
    if (lowerDesc.includes('history') || lowerDesc.includes('ancient') || lowerDesc.includes('war') || lowerDesc.includes('civilization') || lowerDesc.includes('empire') || lowerDesc.includes('historical')) return 'history';
    if (lowerDesc.includes('science') || lowerDesc.includes('physics') || lowerDesc.includes('chemistry') || lowerDesc.includes('biology') || lowerDesc.includes('atom') || lowerDesc.includes('molecule')) return 'science';
    if (lowerDesc.includes('space') || lowerDesc.includes('planets') || lowerDesc.includes('astronomy') || lowerDesc.includes('solar system') || lowerDesc.includes('astronaut')) return 'science';
    if (lowerDesc.includes('technology') || lowerDesc.includes('computer') || lowerDesc.includes('ai') || lowerDesc.includes('robot') || lowerDesc.includes('digital')) return 'technology';
    if (lowerDesc.includes('math') || lowerDesc.includes('algebra') || lowerDesc.includes('geometry') || lowerDesc.includes('calculus') || lowerDesc.includes('equation')) return 'mathematics';
    if (lowerDesc.includes('art') || lowerDesc.includes('painting') || lowerDesc.includes('sculpture') || lowerDesc.includes('music') || lowerDesc.includes('culture')) return 'art';
    return 'general';
  }
  ...
  static calculateSpherePosition(index, total) {
    ...
    return { x: Math.cos(angle) * radius, y: Math.sin(angle * 0.5) * 2, z: Math.sin(angle) * radius };
  }
