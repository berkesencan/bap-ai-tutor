require('dotenv').config();
const fs = require('fs');

async function replaceQuestionsEnhanced() {
  console.log('🚀 Step 2.5 ENHANCED: Replace Questions with Proper Table Formatting');
  console.log('================================================================');
  
  try {
    // Read the successful simple LaTeX
    const successfulLatexPath = 'simple-gemini-output/gemini-generated.tex';
    if (!fs.existsSync(successfulLatexPath)) {
      throw new Error('Successful simple LaTeX not found.');
    }
    
    // Read the new questions from Step 1.5
    const newQuestionsPath = 'step1.5-output/new-questions-text.txt';
    if (!fs.existsSync(newQuestionsPath)) {
      throw new Error('Step 1.5 new questions not found.');
    }
    
    let originalLatex = fs.readFileSync(successfulLatexPath, 'utf-8');
    let newQuestionsText = fs.readFileSync(newQuestionsPath, 'utf-8');
    
    console.log(`📄 Original LaTeX: ${originalLatex.length} characters`);
    console.log(`📝 New questions: ${newQuestionsText.length} characters`);
    
    // Extract just the problems from the new questions text
    const problemsStart = newQuestionsText.indexOf('Problem 1');
    if (problemsStart === -1) {
      throw new Error('Could not find Problem 1 in new questions');
    }
    
    const newProblemsText = newQuestionsText.substring(problemsStart).trim();
    console.log(`🎯 Extracted problems text: ${newProblemsText.length} characters`);
    
    // Find where the problems start in the original LaTeX
    const latexProblemsStart = originalLatex.indexOf('\\section*{Problem 1}');
    if (latexProblemsStart === -1) {
      throw new Error('Could not find Problem 1 section in original LaTeX');
    }
    
    // Find where the problems end
    const latexProblemsEnd = originalLatex.indexOf('\\end{document}');
    if (latexProblemsEnd === -1) {
      throw new Error('Could not find end of document in original LaTeX');
    }
    
    // Extract header and footer
    const latexHeader = originalLatex.substring(0, latexProblemsStart);
    const latexFooter = originalLatex.substring(latexProblemsEnd);
    
    // Convert problems to LaTeX with enhanced table handling
    let newProblemsLatex = convertTextToLatexEnhanced(newProblemsText);
    
    // Combine everything
    const finalLatex = latexHeader + newProblemsLatex + latexFooter;
    
    // Save the result
    if (!fs.existsSync('step2.5-output')) {
      fs.mkdirSync('step2.5-output');
    }
    
    const outputPath = 'step2.5-output/latex-with-new-questions-enhanced.tex';
    fs.writeFileSync(outputPath, finalLatex);
    
    console.log(`✅ Saved enhanced LaTeX to: ${outputPath}`);
    console.log('🎉 Step 2.5 ENHANCED completed successfully!');
    
  } catch (error) {
    console.error('❌ Error:', error.message);
    process.exit(1);
  }
}

function convertTextToLatexEnhanced(problemsText) {
  let latex = '';
  const lines = problemsText.split('\n');
  let inProblemContent = false;
  let inTable = false;
  let tableRows = [];
  
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i].trim();
    
    if (!line) {
      // If we're ending a table, process it
      if (inTable && tableRows.length > 0) {
        latex += generateLatexTable(tableRows);
        tableRows = [];
        inTable = false;
      }
      if (inProblemContent) latex += '\n\n';
      continue;
    }
    
    // Check if this is a problem header
    const problemMatch = line.match(/^Problem (\d+)$/);
    if (problemMatch) {
      const problemNum = problemMatch[1];
      latex += `\\section*{Problem ${problemNum}}\n`;
      inProblemContent = true;
      inTable = false;
      continue;
    }
    
    // Handle problem content
    if (inProblemContent) {
      // Detect table start (Task, Time Taken, etc.)
      if (line.includes('Task') && (line.includes('Time Taken') || lines[i+1]?.includes('Time Taken'))) {
        inTable = true;
        tableRows = [];
        // Look ahead to collect all table rows
        let j = i;
        while (j < lines.length && (lines[j].trim() || inTable)) {
          const currentLine = lines[j].trim();
          if (!currentLine) {
            if (tableRows.length > 0) break;
            j++;
            continue;
          }
          
          // Check if this looks like a table row (single letter/number or short text)
          if (currentLine.match(/^[A-Z]$/) || currentLine.match(/^\d+$/) || 
              currentLine.includes('Task') || currentLine.includes('Time Taken') ||
              currentLine.includes('core type')) {
            tableRows.push(currentLine);
          } else {
            break;
          }
          j++;
        }
        
        // Generate the table and skip the processed lines
        latex += generateLatexTable(tableRows);
        i = j - 1; // Skip the processed lines
        inTable = false;
        tableRows = [];
        continue;
      }
      
      // If not in table, process normal content
      if (!inTable) {
        // Escape LaTeX special characters
        let escapedLine = line
          .replace(/&/g, '\\&')
          .replace(/_/g, '\\_')
          .replace(/%/g, '\\%')
          .replace(/\$/g, '\\$')
          .replace(/#/g, '\\#');
        
        // Handle point indicators
        escapedLine = escapedLine.replace(/\[(\d+)( points?)?\]/g, '\\textbf{[$1 points]}');
        
        // Handle sub-questions
        if (escapedLine.match(/^[a-z]\./)) {
          latex += `\\textbf{${escapedLine}}\n\n`;
        } else if (escapedLine.match(/^\d+\./)) {
          latex += `\\textbf{${escapedLine}}\n\n`;
        } else {
          latex += `${escapedLine}\n\n`;
        }
      }
    }
  }
  
  return latex;
}

function generateLatexTable(tableRows) {
  if (tableRows.length === 0) return '';
  
  console.log(`🔧 Generating table with ${tableRows.length} rows:`, tableRows);
  
  // Try to identify the table structure
  // Expected: Task, Time Taken on core type A, Time Taken on core type B, E, F, G, H, I, 8, 12, 18, 6, 11, 7, 9, 22, 4, 6
  
  let latex = '\n\\begin{center}\n';
  latex += '\\begin{tabular}{|c|c|c|}\n';
  latex += '\\hline\n';
  
  // Header row
  latex += '\\textbf{Task} & \\textbf{Time Taken on core type A} & \\textbf{Time Taken on core type B} \\\\\n';
  latex += '\\hline\n';
  
  // Find where the actual data starts (after headers)
  let dataStartIdx = 0;
  for (let i = 0; i < tableRows.length; i++) {
    if (tableRows[i].match(/^[A-Z]$/) && i + 1 < tableRows.length && tableRows[i + 1].match(/^[A-Z]$/)) {
      dataStartIdx = i;
      break;
    }
  }
  
  // Extract tasks (E, F, G, H, I) and their corresponding values
  const tasks = [];
  const typeAValues = [];
  const typeBValues = [];
  
  let taskIdx = dataStartIdx;
  while (taskIdx < tableRows.length && tableRows[taskIdx].match(/^[A-Z]$/)) {
    tasks.push(tableRows[taskIdx]);
    taskIdx++;
  }
  
  // Find the numeric values (should be 5 values for type A, then 5 for type B)
  let valueIdx = taskIdx;
  let valueCount = 0;
  while (valueIdx < tableRows.length && tableRows[valueIdx].match(/^\d+$/)) {
    if (valueCount < tasks.length) {
      typeAValues.push(tableRows[valueIdx]);
    } else {
      typeBValues.push(tableRows[valueIdx]);
    }
    valueCount++;
    valueIdx++;
  }
  
  // Generate table rows
  for (let i = 0; i < tasks.length; i++) {
    const task = tasks[i] || '';
    const typeA = typeAValues[i] || '';
    const typeB = typeBValues[i] || '';
    latex += `${task} & ${typeA} & ${typeB} \\\\\n`;
    latex += '\\hline\n';
  }
  
  latex += '\\end{tabular}\n';
  latex += '\\end{center}\n\n';
  
  return latex;
}

// Run the function
replaceQuestionsEnhanced().catch(console.error);

 
 
 
 
 
 
 