...
\begin{enumerate}
    ...
    \item (b) $f(n) = 2^n$, $g(n) = n^2$
    \item (c) $f(n) = n \log n$, $g(n) = n \log(n^2)$
    \item (d) $f(n) = 10n^2 + 5n$, $g(n) = n^2$
    \item (e) $f(n) = \sqrt{n}$, $g(n) = \log_2 n$
\end{enumerate}
...
\begin{enumerate}
    ...
    \item (b) $T(n) = 9T(n/3) + n$
    \item (c) $T(n) = T(n/2) + n^2$
\end{enumerate}

\section*{Problem 4}
Our Master Theorem can be applied to a recurrence of the form $T(n) = aT(n/b) + n^d$, where $a, b, d$ are constants with $a > 0$, $b > 1$, $d > 0$. Consider instead a recurrence of the form $T_{new}(n) = aT_{new}(n/b) + n \log n$ where $a > 0$, $b > 1$ (and $T(1) = 1$).
...
\begin{enumerate}
    ...
    \item (b) $T_{new}(n) = \Omega(n \log^2 n)$ if $\log_b a = 1$
\end{enumerate}
