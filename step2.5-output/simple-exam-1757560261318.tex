...
(c) $f(n) = n^2 \log n$, $g(n) = n^3$
(d) $f(n) = 7 \sum_{i=1}^n i^2$, $g(n) = n^3$
(e) $f(n) = 2^{\log_2 n}$, $g(n) = n$


\section*{Problem 2}
Give a formal proof of the following statement: If $f(n) \ge 1$ for all $n \in \mathbb{N}$, $g(n) \ge 1$ for all $n \in \mathbb{N}$, $f(n) = O(g(n))$, and $g(n)$ is unbounded (meaning $\lim_{n \to \infty} g(n) = \infty$) then $\log_2 (f(n))^2 = O(\log (g(n))^2)$.
...
\section*{Problem 4}
Our Master Theorem can be applied to a recurrence of the form $T(n) = aT(n/b) + n^d$, where $a, b, d$ are constants with $a > 0$, $b > 1$, $d > 0$. Consider instead a recurrence of the form $T_{\text{new}}(n) = aT_{\text{new}}(n/b) + n \log_d n$ where $a > 0$, $b > 1$, $d > 1$ (and $T(1) = 1$).
...
(a) $T_{\text{new}}(n) = O(n^2)$ if $\log_b a < 1$
(b) $T_{\text{new}}(n) = \Omega(n \log n)$ if $\log_b a = 1$
...
(a)  Prove that this algorithm does \textbf{not} correctly sort the array in general. Provide a counterexample.
...
\section*{Problem 8}
Let $f(n) = n \log n$ and $g(n) = n^{1.1}$. Prove that $f(n) = O(g(n))$.


\section*{Problem 9}
Solve the recurrence $T(n) = 3T(n/2) + n$ using the Master Theorem.  Assume $T(1) = 1$. Show your work and state your answer in $\Theta$ notation.


\section*{Problem 10}
Consider the recurrence $T(n) = 2T(\frac{n}{4}) + \sqrt{n}$.  Can Our Master Theorem be applied? If so, what is the solution in $\Theta$ notation? If not, explain why not.
