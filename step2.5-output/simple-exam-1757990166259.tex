...
(a) $f(n) = 2^n$, $g(n) = n!$
(b) $f(n) = n^2 + 3n$, $g(n) = n^2$
(c) $f(n) = \log_2 n$, $g(n) = \sqrt{n}$
(d) $f(n) = 7 \sum_{i=1}^n i^2$, $g(n) = n^3$
(e) $f(n) = \log_2 (n^3)$, $g(n) = \log_4 (n^5)$
...
Note that a similar statement with exponential functions is not true: If $f(n) = 2^n$ and $g(n) = n$, then $f(n) = O(2^n)$, but $2^{2^n}$ is not $O(2^n)$.
