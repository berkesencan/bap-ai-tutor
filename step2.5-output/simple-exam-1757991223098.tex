...
\begin{enumerate}
    ...
    \item Our Master Theorem can be applied to a recurrence of the form $T(n) = aT(n/b) + n^d$, where $a, b, d$ are constants with $a > 0$, $b > 1$, $d > 0$. Consider instead a recurrence of the form $T_{new}(n) = aT_{new}(n/b) + n^{\log_b a} \log n$ where $a > 0$, $b > 1$ (and $T(1) = 1$).
    ...
    \begin{enumerate}
        \item $T_{new}(n) = O(n^{\log_b a} (\log n)^2)$
    ...
    \end{itemize}
    This algorithm does not sort the array, but what does it do?  Describe the final state of the array,  and justify your answer.  Then, give a recurrence relation for its running time.  Solve this recurrence using the Master Theorem.

    \item  Design a divide-and-conquer algorithm to find the largest and second largest elements in an unsorted array of size $n$.  Give a recurrence for the running time of your algorithm, and solve it using Our Master Theorem (or other appropriate methods).  Analyze the running time and compare it to a simple linear-scan approach.
