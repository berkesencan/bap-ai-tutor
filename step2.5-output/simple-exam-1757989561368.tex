...
\begin{enumerate}
    ...
    \item Our Master Theorem can be applied to a recurrence of the form $T(n) = aT(n/b) + n^d$, where $a, b, d$ are constants with $a > 0$, $b > 1$, $d \ge 0$. Consider instead a recurrence of the form $T_{new}(n) = aT_{new}(n/b) + n^d \log n$ where $a > 0$, $b > 1$, $d \ge 0$ (and $T(1) = 1$).
    ...
    \begin{enumerate}
        ...
        \item $T_{new}(n) = \Omega(n^d \log n)$ if $\log_b a = d$
    ...
    \begin{itemize}
        \item If the input is a single integer, return the integer.
        \item Otherwise, recursively process the first half of the input. Recursively process the second half of the input. Then, add the results together. The input is a list of integers.
    \end{itemize}
    \begin{enumerate}
