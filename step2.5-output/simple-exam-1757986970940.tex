...
\section*{Problem 8}
Let $f(n) = n^2 + 10n \log n$ and $g(n) = n^2$.  Determine whether $f(n) = O(g(n))$, $f(n) = \Omega(g(n))$, $f(n) = \Theta(g(n))$, or none of the above. Justify your answer using the formal definitions of big-O, big-Omega, and big-Theta.

\section*{Problem 9}
Solve the recurrence relation $T(n) = 3T(n/2) + n$ using the Master Theorem. Show your work. What is the running time in $\Theta$ notation? Assume $T(1) = 1$.

\section*{Problem 10}
Consider the recurrence $T(n) = T(n-1) + n$.  Solve this recurrence using iteration (unrolling the recursion).  Express your solution in $\Theta$ notation.  Assume $T(1)=1$.
